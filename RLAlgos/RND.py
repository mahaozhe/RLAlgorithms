...
class RND:
    ...
    def save(self, indicator="final"):
        torch.save(self.agent.state_dict(),
                   os.path.join(self.save_folder, "agent-{}-{}-{}.pth".format(self.exp_name, indicator, self.seed)))
...
class RND_Atari:
    ...
    def __init__(self, envs, agent_class, rn_class, exp_name="rnd", seed=1, cuda=0, gamma=0.99, int_gamma=0.99,
        ...
        self.optimizer = optim.Adam(self.combined_parameters, lr=lr, eps=eps)
        self.reward_rms = RunningMeanStd()
        # * only use the last frame from the stacked frames
    ...
    def learn(self, total_timesteps=500000):
        ...
        for step in range(self.rollout_length * self.num_iterations_obs_norm_init):
            ...
            obs_norm_acs = self.envs.action_space.sample()
            s, _, _, _, _ = self.envs.step(obs_norm_acs)
            obs_norm_obs.append(s[:, 3, :, :].reshape([-1, 1, 84, 84]))
        ...
        # the number of updates = total_timesteps // (rollout_length * num_envs)
        num_updates = int(total_timesteps // self.big_batch_size)
        ...
        for update in range(1, num_updates + 1):
            ...
            for step in range(self.rollout_length):
                ...
                rnd_next_obs = (((next_obs[:, 3, :, :].reshape(self.num_envs, 1, 84, 84) - torch.from_numpy(
                    self.obs_rms.mean).to(self.device)) / torch.sqrt(
                    torch.from_numpy(self.obs_rms.var).to(self.device))).clip(-5, 5).float())
    ...
    def optimize(self, global_step, next_obs, next_done):
        ...
        rnd_next_obs = (((b_obs[:, 3, :, :].reshape(-1, 1, 84, 84) - torch.from_numpy(self.obs_rms.mean).to(
            self.device)) / torch.sqrt(torch.from_numpy(self.obs_rms.var).to(self.device))).clip(-5, 5).float())
