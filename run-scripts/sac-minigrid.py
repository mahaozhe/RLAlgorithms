"""
The script to run SAC on MiniGrid environments (discrete action space).
"""
...
from Networks.ActorNetworks import SACActorMiniGrid
from Networks.QNetworks import SACSoftQNetworkMiniGrid

from utils.env_makers import minigrid_env_maker


def parse_args():
    parser = argparse.ArgumentParser(description="Run SAC on MiniGrid environments.")

    parser.add_argument("--exp-name", type=str, default="sac-minigrid")

    parser.add_argument("--env-id", type=str, default="MiniGrid-Large-Empty-Room")
    ...
    parser.add_argument("--save-folder", type=str, default="./sac-minigrid/")
...
def run():
    ...
    agent = SAC_Atari(env=env, actor_class=SACActorMiniGrid, critic_class=SACSoftQNetworkMiniGrid,
                      exp_name=args.exp_name, seed=args.seed, cuda=args.cuda, gamma=args.gamma,
                      buffer_size=args.buffer_size, rb_optimize_memory=args.rb_optimize_memory,
                      batch_size=args.batch_size, policy_lr=args.policy_lr, q_lr=args.q_lr, eps=args.eps,
                      alpha_lr=args.alpha_lr, target_network_frequency=args.target_network_frequency, tau=args.tau,
